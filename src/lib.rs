...
pub use tracing;
pub use archy_macros::{Service, service, traced, untraced, startup, shutdown};
...
pub mod prelude {
    pub use crate::{
        ...
        service, traced, untraced, startup, shutdown,
    };
}
...
pub trait Service: Sized + Send + Sync + 'static {
    ...
    /// Async initialization - runs after create, before workers start receiving messages.
    /// Default: no-op.
    ///
    /// **Note:** Cannot call other services here - workers aren't running yet.
    fn startup(self: Arc<Self>) -> impl Future<Output = ()> + Send { async {} }

    /// Message handler
    fn handle(self: Arc<Self>, msg: Self::Message) -> impl Future<Output = ()> + Send;

    /// Cleanup - runs on shutdown, before channels close.
    /// Default: no-op.
    ///
    /// **Note:** Can call other services - workers are still running.
    fn shutdown(self: Arc<Self>) -> impl Future<Output = ()> + Send { async {} }
}
